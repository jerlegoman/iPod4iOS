...
- (id)initWithManagedObjectContext:(NSManagedObjectContext *)managedObjectContext {
	self = [super initWithItems:@[@"Artists", @"Albums", @"Songs", @"Playlists", @"Genres", @"Composers", @"Brightness +", @"Brightness -", @"Clock"]];
	if (!self) return nil;
...
- (void)clickWheelCenterButtonTapped {
    ...
    } else if ([title isEqualToString:@"Brightness +"]) {
        [UIScreen mainScreen].brightness = 1.0;
    
    } else if ([title isEqualToString:@"Brightness -"]) {
        [UIScreen mainScreen].brightness = 0.3;
        
    } else if ([title isEqualToString:@"Clock"]) {
        UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:@"Sorry" message:@"This Feature isn't avalable yet" delegate:self cancelButtonTitle:@"Close this message" otherButtonTitles:@"Ok", nil];
        [alertView show];
